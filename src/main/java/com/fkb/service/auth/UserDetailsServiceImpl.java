...
import com.fkb.exception.DuplicateUserException;
...
public class UserDetailsServiceImpl implements EnhancedUserDetailsService {
	...
	public final UserDetails loadUserByUsername(final String username) {
		...
		if (user == null) {
			final Query query = entityManager
					.createQuery("SELECT u FROM UserAccount u WHERE username = :username");
	...
	public final void addUser(final UserAccount user, final Locale locale) {
		final UserAccount cachedUser = (UserAccount) memcacheService.get(user
				.getUsername());
		...
		final TaskOptions taskOptions = TaskOptions.Builder
				.withUrl(mailTaskUrl).param("username", user.getUsername())
	...
	public final boolean activateUser(final String key) {
		final Query query = entityManager
				.createQuery("SELECT u FROM UserAccount u WHERE activationKey = :key");
